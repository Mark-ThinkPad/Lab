...
from EmotionAnalysis.paths import EA_DIR

start = time.perf_counter()

# 部署时更改为服务器上的绝对路径
jieba.load_userdict(EA_DIR + '/dict.txt')
data = pd.read_csv(EA_DIR + '/try20.csv')
# data = data[int(len(data)*0.80):]
pos = (open(EA_DIR + "/pos.txt", 'r', encoding="utf-8")).readlines()
neg = (open(EA_DIR + "/neg.txt", 'r', encoding="utf-8")).readlines()
neu = (open(EA_DIR + "/neu.txt", 'r', encoding="utf-8")).readlines()
stop = (open(EA_DIR + "/stopword.txt", 'r', encoding="utf-8")).readlines()
output = (open(EA_DIR + "/output.txt", 'w', encoding="utf-8"))
